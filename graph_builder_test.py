...
def build_graph():
    ...
    start        = Zone("start",        0,  0,  zone_type=ZoneType.NORMAL,     max_drones=25)
    ...
    # Layer 0 — bottleneck gates (capacity=1 on each)
    graph.connect("start",      "gate_hell1",  capacity=1)
    # NOTE: "connection: -" in source is malformed/intentional noise — skipped
    graph.connect("gate_hell2", "gate_hell3",  capacity=1)
    graph.connect("gate_hell3", "gate_hell4",  capacity=1)
    graph.connect("gate_hell4", "gate_hell5",  capacity=1)
