...
class Zone:
    ...
    def __init__(
        ...
        y: int,
        color: str | None = None,
        zone_type: str | ZoneType = ZoneType.NORMAL,
        max_drones: int = 1,
    ) -> None:
        ...
        self.zone_cost = self.zone_state_cost[self.zone_type]
        self.color: str | None = color
        self.max_drones: int = max_drones
        self.current_drones: int = 0
        self.neighbors: list["Zone"] = []
        self.drones_in_station: list[Drone] = []
...
class Graph:
    ...
    def create_drones(self, drones_number: int) -> None:
        if self.start is None:
            raise RuntimeError("Cannot create drones before start hub is set")
        for i in range(drones_number):
            self.drones.append(Drone(i, "WAITING", self.start.name, None))

    def build_graph(self, map: str) -> None:
