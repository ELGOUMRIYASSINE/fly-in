...
def validate_meta_value(meta_key: str, meta_value: str, entry_type: str) -> None:
    zone_values = ["restricted", "normal", "blocked", "priority"]
    meta_key = meta_key.strip()
    if meta_key == "color" and entry_type == "hub":
        if not re.match(r'^[a-zA-Z]+$', meta_value):
            raise ParsingError(
                f"Invalid color value '{meta_value}'. Color must be a string of letters.")
    elif meta_key == "max_drones" and entry_type == "hub":
        if not re.match(r'^\d+$', meta_value) or int(meta_value) <= 0:
            raise ParsingError(
                f"Invalid max_drones value '{meta_value}'. max_drones must be a positive integer.")
    elif meta_key == "zone" and entry_type == "hub":
        if meta_value not in zone_values:
            raise ParsingError(
                f"Invalid zone value '{meta_value}'. Zone must be one of: {', '.join(zone_values)}")
    elif meta_key == "max_link_capacity" and entry_type == "connection":
        if not re.match(r'^\d+$', meta_value) or int(meta_value) <= 0:
            raise ParsingError(
                f"Invalid max_link_capacity value '{meta_value}'. "
                "max_link_capacity must be a positive integer."
            )
    else:
        raise ParsingError(f"Unknown metadata key '{meta_key}' for '{entry_type}'.")


def validate_connection(value: str, use_default: bool) -> None:
    ...
    if "-" not in hub_part:
        raise ParsingError(
            "Connection definition must contain a '-' character "
            "to separate 'from' and 'to' hubs"
        )
    if hub_part.count("-") > 1:
        raise ParsingError(
            "Connection definition must contain only one '-' character "
            "to separate 'from' and 'to' hubs"
        )
...
def validate_order(line: str) -> bool:
    use_default = False
    ...
    if ("[" not in value and "]" in value) or ("[" in value and "]" not in value):
        raise ParsingError(
            f"{key.capitalize()} definition (metadata) must contain metadata "
            "enclosed in square brackets []"
        )
    if value.count("[") > 1 or value.count("]") > 1:
        raise ParsingError(
            f"{key.capitalize()} definition (metadata) must contain only one "
            "pair of square brackets []"
        )
    if value.count("[") == 1 and value.count("]") == 1 and value.index("[") > value.index("]"):
        raise ParsingError(
            f"{key.capitalize()} definition (metadata) must have the format: "
            "name x y [key=value ...]"
        )
    if "[" in value:
    ...
    if key in ["hub", "start_hub", "end_hub"]:
        ...
        if len(parts) != 3:
            raise ParsingError(
                "Hub definition must have exactly three fields: name, x, y before the metadata")
        _, raw_x, raw_y = parts
        try:
            int(raw_x)
            int(raw_y)
        ...
        if not use_default:
            ...
            if meta:
                for item in meta.split():
                    if "=" not in item:
                        raise ParsingError(
                            f"Invalid metadata format '{item}'. Expected key=value pairs.")
            ...
            if "hub" in key:
                ...
                if meta_raw:
                    for item in meta_raw.split():
                        ...
                        zone_data = hub_data["zone"]
                        if sub_key == "zone":
                            zone_data["type"] = sub_value
                        elif sub_key == "color":
                            zone_data["color"] = sub_value
                        elif sub_key == "max_drones":
                            zone_data["max_drones"] = int(sub_value)
